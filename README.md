...
This is a set of Python scripts to analyze logs. There is a set of logs files with streams in different countries by different users at a given date. Theses scripts allow to compute two files by day : one with the streams by country and one with the streams by user.
